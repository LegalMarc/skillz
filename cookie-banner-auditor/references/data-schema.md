...
| `agent_verdict` | Set when a supplied verdict was consulted: `applied`, `decision`, `selector`, `rationale`, `matched_by`, `rejected_reason`, `prior_decision`, and `rejected_detail` when refused. A verdict is re-resolved and re-vetoed before it is acted on, so a rejected one records why. `prior_decision` is the resolution's own `decision` from just before this verdict was consulted (`conflict` / `vetoed` / `unresolved`) - the value a successful verdict goes on to overwrite with `agent_verdict` at the top level, kept here so a control's original ambiguity is never lost once an operator resolves it. |
