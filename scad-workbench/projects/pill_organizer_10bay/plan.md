...
| 7 | The vault: the crossing chute's 40-degree ceiling gabled across each bay so its face is inside the 45-degree overhang limit. Every external edge and corner rounded or chamfered. Then the independent review of revision 7 found the vault had thinned the hopper divider to 0.2 mm at its foot, and that fix plus the review's other findings (rail undersides, barb angle, fillet shoulders) are the second commit of this revision | D26–D28 | `91c9f92`, this branch |
...
| D16 | Decision | Ordinary | The wall between the two fill mouths leans 8.8mm forward at the rim, pivoting where it springs off the chute ceiling OR the end of hopper B's ramp, whichever is higher (D28). The mouths were 70mm and 32mm because the two rows' VOLUMES were held near each other; the number you actually meet, with a bottle in your hand, is the mouth. Leaning brings them to about 3:2: 62.1 / 39.9 at 17.3 degrees in revision 4, 62.7 / 39.3 at 26.0 degrees from revision 7's review fix, when the spring point rose to the ramp's end. It costs hopper B a wedge above its own floor and gives hopper A the same wedge, and it gives hopper A a mouth wider than its throat, which is the right way round for a hopper. | Confirmed | User instruction, this session |
...
| D26 | Decision | Critical | The chute ceiling on the 40-degree leg is vaulted: a shallow gable across each bay, ridge 6mm above the plain ceiling at the bay centre, edges 6mm below it at the dividers (gable 29.2 degrees). A face sloping 40 degrees front-to-back and 29.2 degrees sideways is 44.8 degrees from vertical -- inside the conservative FDM limit -- where the plain ceiling was 50, the one ADVISORY the body carried since revision 3. The rise is split so neither side pays it all: the ridge goes up into the deck, so hopper B's ramp starts 6mm above tray B's floor (a riser at the tray's back wall; the internal fillet rounds its FOOT, its top edge is a sharp convex edge, which does not hang pills) and its outlet rises with it (outlet_h 30 -> 28 to keep 4mm under the rim); the edges come down, leaving 30mm clear at the dividers and 42 at the ridge. The porch is not vaulted -- the rib carries it. The ridge void starts 1mm inside each end of the added roof solid so no faces coincide. Cost: row B 182 -> 166 mL per bay, still 1.13x the 90-day charge. | Confirmed | User instruction, this session; face-normal scan and contains() probes |
| D27 | Decision | Ordinary | Edges a hand meets are rounded or chamfered. Body: the four vertical corners rounded at 3mm (a cutter overstepping outward, so no cut face lands on the body's own faces), the silhouette's convex corners in section rounded at 1.5 (an opening pass on OUTER, with the base corners put back square for the first layers) -- which is a real 1.5 round on the step and back edges and only a 0.16mm easing on the obtuse front edge of the pick plane; the side walls' long top edges at x = 0 and 230 are NOT treated, since they run along the extrusion. Lids: plan-view corners rounded at 2, the fill lid's top perimeter chamfered 1mm (each convex outline hulled with its own inset, the lip flush on top and ending inside the plate's chamfer band), the pick lid's plate and skirt edges chamfered in section (its top face goes on the bed, where a round is a 90-degree overhang) and at its x-ends. Internal flow corners already carry the 2mm fillet from revision 1. | Confirmed | User instruction, this session; scope corrected by the revision 7 review |
| D28 | Decision | Critical | The hopper divider springs from the higher of the chute ceiling and the end of hopper B's ramp, and VOID_A carries a vertex there so the wall is vertical up to it. D26 raised the ramp's end 7mm above the old spring point, and a wall that starts leaning below the ramp's end crosses the ramp's own face: the review measured it 0.2-0.8mm thick over its bottom 5mm in every bay, and `check_printability.py`'s thin-wall half had reported it inside the ADVISORY that was being dismissed for its overhang half. Asserted now on the wall thickness at the ramp's end. Also from that review: the male rails' undersides are 45 degree chamfers instead of flat faces 10mm above the bed; the barb's return face is defined by its slope through the tab's outer face (it was 25.7 degrees, not the 35 D23 states); `void_top_over` 1 -> 5 so the fillet pass no longer puts shoulders back under the pick plane; the pick lid's notch polygons start 6mm below the skirt so their rounding does not feather the skirt edge; the back seat-ledge relief oversteps into the back wall like the front one; the back groove's bore ends above the rim. | Confirmed | Independent review of revision 7, this session |
